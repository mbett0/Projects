enum States{INIT, S0, S1, S0Press, S1Press} SM1_state = INIT;
int X_button = 8;
int Y_button = 9;
...
void writeS1(unsigned char b_temp, int size = b_size)
{
...
void writeS0(unsigned char b_temp, int size = b_size)
{
   for (int i = (size - 1); i >= 0; i--) {
        if (((b_temp >> i) & 0x01)) {
        digitalWrite(b[i], HIGH);
